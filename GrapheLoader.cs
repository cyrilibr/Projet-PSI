    ...
    public static class GrapheLoader
    {
        ...
        public static Graphe<Station> ChargerDepuisBDD()
        {
            ...
            foreach (var raw in listeBrute)
            {
                ...
                {
                    // En théorie, tous les ID bruts sont mappés, mais on protège au cas où
                    continue;
                }

                // Si une station précédente existe et est mappée, on crée le lien
                if (raw.Precedent.HasValue
                    && mapIdBddVersUnifie.TryGetValue(raw.Precedent.Value, out int vPrev)
                    && u != vPrev)
                {
                    // Lien orienté : de la précédente vers la courante
                    graphe.AjouterLien(vPrev, u, poids: 1);
                }

                // Si une station suivante existe et est mappée, on crée le lien
                if (raw.Suivant.HasValue
                    && mapIdBddVersUnifie.TryGetValue(raw.Suivant.Value, out int vNext)
                    && u != vNext)
                {
                    // Lien orienté : de la courante vers la suivante
                    graphe.AjouterLien(u, vNext, poids: 1);
                }
            }
